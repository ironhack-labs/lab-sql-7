...
# include this last name in our output. Last name "Astaire" is present only one time with actor "Angelina Astaire", hence we would want this in our output list.
...
select count(rental_id), staff_id from sakila.rental group by staff_id;

#4. Using the film table, find out how many films were released each year.

select count(film_id), release_year from sakila.film group by release_year;

#5. Using the film table, find out for each rating how many films were there.

select count(film_id), rating from sakila.film group by rating;

#6. What is the mean length of the film for each rating type. Round off the average lengths to two decimal places

select round(avg(length),2) as av_length, rating from sakila.film group by rating;

#7. Which kind of movies (rating) have a mean duration of more than two hours?

select round(avg(length),2) as av_length, rating from sakila.film group by rating having av_length > 120;
